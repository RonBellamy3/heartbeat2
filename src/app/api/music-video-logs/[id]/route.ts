...
import { updateMusicVideoLogSchema } from "@/lib/validation/music-video";
import { rateLimit, requestIp, RATE_LIMITS } from "@/lib/rate-limit";

async function getOwnedLog(id: string, userId: string) {
  const log = await prisma.musicVideoLog.findUnique({ where: { id } });
...
export async function PATCH(
  ...
  const parsed = updateMusicVideoLogSchema.safeParse(body);
  if (!parsed.success) {
  ...
  const watchedOn = parsed.data.watchedOn ? new Date(parsed.data.watchedOn) : undefined;
  if (watchedOn && Number.isNaN(watchedOn.getTime())) {
    return NextResponse.json({ error: "Invalid date" }, { status: 400 });
  }

  const log = await prisma.musicVideoLog.update({
    where: { id },
    data: {
...
export async function DELETE(
  ...
  await prisma.musicVideoLog.update({
    where: { id },
